...
{
    /**
     * Ambil dan proses data laporan.
     * Digunakan bersama oleh index() dan print().
     */
            ...
            foreach ($daterange as $date) {
                ...
                foreach ($karyawans as $k) {
                    ...
                    $pengajuan = $allPengajuan->where('user_id', $k->user_id)
                        ->filter(fn($item) => $tgl >= $item->tanggal_mulai && $tgl <= $item->tanggal_selesai)->first();
                    $jadwal    = JadwalKerja::with('shift')->where('user_id', $k->user_id)
                        ->where('hari', $hariNama)->where('status', 'aktif')->first();
