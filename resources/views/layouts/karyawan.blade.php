    ...
    <div class="flex h-screen w-screen overflow-hidden relative">
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-72 bg-indigo-950 text-white flex flex-col shadow-2xl transform -translate-x-full md:translate-x-0 md:relative transition-transform duration-300 ease-in-out">
            ...
            <nav class="flex-1 p-4 space-y-2 mt-4 overflow-y-auto">
                ...
                <a href="{{ route('karyawan.laporan.index') }}" class="flex items-center p-3 rounded-xl transition {{ request()->routeIs('karyawan.laporan.*') ? 'bg-indigo-600 shadow-lg text-white' : 'text-indigo-200 hover:bg-indigo-900 hover:text-white' }}">
                    <i class="fas fa-file-alt w-6 text-center"></i>
                    <span class="ml-3 font-bold text-sm uppercase">Laporan Saya</span>
                </a>
                ...
                {{-- Dompet Integritas --}}
                <p class="px-4 text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mt-6 mb-2 sidebar-text">
                    Integritas</p>
