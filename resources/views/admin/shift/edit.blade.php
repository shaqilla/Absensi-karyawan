...
<div class="w-full pb-6">
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 md:mb-8 gap-2">
        <div>
            <h1 class="text-xl md:text-3xl font-black text-gray-800 uppercase tracking-tighter">Edit Shift</h1>
            <p class="text-gray-500 text-[10px] md:text-sm italic font-medium">Ubah pengaturan operasional dan jam kerja shift ini.</p>
        </div>
    ...
    <div class="bg-white rounded-2xl md:rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
        {{-- ACTION diarahkan ke update dengan ID shift --}}
        <form action="{{ route('admin.shift.update', $shift->id) }}" method="POST" class="p-5 md:p-10">
            @csrf
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <select name="nama_shift" class="w-full border-gray-200 rounded-xl p-3 md:p-4 focus:ring-2 focus:ring-indigo-500 border outline-none font-bold text-gray-700 text-sm md:text-base bg-gray-50" required>
                            ...
                            @php $currentShift = old('nama_shift', $shift->nama_shift); @endphp
                            <option value="Shift Pagi" {{ $currentShift == 'Shift Pagi' ? 'selected' : '' }}>Shift Pagi</option>
                            <option value="Shift Siang" {{ $currentShift == 'Shift Siang' ? 'selected' : '' }}>Shift Siang</option>
                            <option value="Shift Malam" {{ $currentShift == 'Shift Malam' ? 'selected' : '' }}>Shift Malam</option>
                            <option value="Shift Lembur" {{ $currentShift == 'Shift Lembur' ? 'selected' : '' }}>Shift Lembur</option>
                        </select>
                    </div>

                    <div>
                        <label class="block text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Toleransi Telat (Menit)</label>
                        <input type="number" name="toleransi_telat" value="{{ old('toleransi_telat', $shift->toleransi_telat) }}"
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="relative">
                            ...
                            {{-- Format jam ke H:i agar sesuai dengan input type="time" --}}
                            <input type="time" name="jam_masuk" value="{{ old('jam_masuk', date('H:i', strtotime($shift->jam_masuk))) }}"
                        ...
                        <div class="relative">
                            ...
                            <input type="time" name="jam_keluar" value="{{ old('jam_keluar', date('H:i', strtotime($shift->jam_keluar))) }}"
                                onclick="this.showPicker()"
                    ...
                    <div class="p-4 bg-amber-50 rounded-2xl border border-amber-100 flex items-start">
                        ...
                        <p class="text-[9px] text-amber-800 font-bold leading-relaxed uppercase">
                            Mengubah jam shift akan mempengaruhi perhitungan presensi karyawan yang menggunakan shift ini mulai dari sekarang.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-8 md:mt-12 flex justify-end border-t border-gray-50 pt-6">
                <button type="submit" class="w-full md:w-auto bg-indigo-600 text-white px-16 py-4 rounded-2xl font-black hover:bg-indigo-700 transition shadow-lg text-xs uppercase tracking-widest">
                    Update Shift
                </button>
            </div>
        </form>
